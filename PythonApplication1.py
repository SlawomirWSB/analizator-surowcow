...
def main():
    ...
    # Dobór odpowiedniego okresu danych do interwału, aby uniknąć dziur
    period_map = {"1m": "1d", "5m": "5d", "15m": "7d", "1h": "30d", "1d": "max"}
    period = period_map.get(interval, "max")

    try:
        df = yf.download(RYNKI[kat][inst], period=period, interval=interval, progress=False)
        ...
        if not df.empty:
            ...
            # Usuwamy rzędy z zerowym obrotem lub brakami cen, co naprawia "dziwne rozkłady"
            df = df[df['Open'] > 0]
            ...
            # Kluczowa zmiana: rangebreaks usuwa puste weekendy i godziny nocne z osi czasu
            fig.update_xaxes(
                rangebreaks=[
                    dict(bounds=["sat", "mon"]), # pomiń weekendy
                    dict(values=["2025-12-25", "2026-01-01"]) # przykładowe dni wolne
                ]
            )
