...
# Styl CSS dla małej czcionki na telefonie
st.markdown("""
    <style>
...
def main():
    ...
    try:
        ...
        if not df.empty:
            ...
            # Sygnały historyczne do znaczników
            ...
            if show_markers:
                ...
                fig.add_trace(go.Scatter(x=buys.index, y=buys['Low']*0.999, mode='markers', marker=dict(symbol='triangle-up', size=8, color='lime'), name='B'), row=1, col=1)
                fig.add_trace(go.Scatter(x=sells.index, y=sells['High']*1.001, mode='markers', marker=dict(symbol='triangle-down', size=8, color='red'), name='S'), row=1, col=1)

            # RSI na dole
            fig.add_trace(go.Scatter(x=df.index, y=df['RSI'], line=dict(color='#00d4ff', width=1)), row=2, col=1)
            fig.add_hline(y=70, line_dash="dash", line_color="red", row=2, col=1)
            fig.add_hline(y=30, line_dash="dash", line_color="green", row=2, col=1)

            # Zoom na ostatnie świece
            zoom = 100 if interval == "1m" else 60
            fig.update_xaxes(range=[df.index[-min(len(df), zoom)], df.index[-1]])
            fig.update_layout(height=450, margin=dict(l=5, r=5, t=5, b=5), template="plotly_dark", xaxis_rangeslider_visible=False, showlegend=False)
